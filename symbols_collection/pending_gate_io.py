...
logger = setup_logger("gate_io", "../log/app.log")
...
def insert_to_db(data, ref):
    ...
    sql = "INSERT INTO symbols (symbol_name, remark) VALUES (%s, 'gate_io')"

    cursor.executemany(sql, inst_ids_tuples)

    connection.commit()
    release_connection(connection)
    logger.info(f"{len(filtered_symbols)} record(s) inserted gate_io")
...
def transform_and_filter_symbols(data, ref):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"gate_io_unmatched_symbols : {unmatched}")
