...
logger = setup_logger("jubi", "../log/app.log")
...
def insert_to_db(data, ref):
    ...
    sql = "INSERT INTO symbols (symbol_name, remark) VALUES (%s, 'jubi')"

    cursor.executemany(sql, inst_ids_tuples)

    connection.commit()
    release_connection(connection)
    logger.info(f"{len(filtered_symbols)} record(s) inserted jubi")
...
def transform_and_filter_symbols(data, ref):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"jubi_unmatched_symbols : {unmatched}")

    with open('jubi_unmatched_symbols.txt', 'w') as file:
        for unmatched in unmatched_symbols:
            file.write(f"jubi_unmatched_symbols : {unmatched}" + '\n')
