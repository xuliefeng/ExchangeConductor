...
logger = setup_logger("bing_x", "../log/app.log")
...
def transform_and_filter_symbols(data, ref):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"bing_x_unmatched_symbols : {unmatched}")

    with open('bing_x_unmatched_symbols.txt', 'w') as file:
        for unmatched in unmatched_symbols:
            file.write(f"bing_x_unmatched_symbols : {unmatched}" + '\n')
