...
logger = setup_logger("kraken", "../log/app.log")
...
def kraken():
    ...
    if response.status_code == 200:
        ...
        if response_pairs.status_code == 200:
            ...
            logger.error(f"Request for AssetPairs failed with status code {response_pairs.status_code}")
    else:
...
def transform_and_filter_symbols(data, kraken_assets, coins_r):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"kraken_unmatched_symbols : {unmatched}")
    ...
    logger.info(f"base_symbols :          {len(kraken_assets)}")
