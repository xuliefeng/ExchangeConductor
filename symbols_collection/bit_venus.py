...
logger = setup_logger("bit_venus", "../log/app.log")
...
def insert_to_db(data, ref):
    ...
    logger.info(f"{len(filtered_symbols)} record(s) inserted bit_venus")
...
def transform_and_filter_symbols(data, ref):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"bit_venus_unmatched_symbols : {unmatched}")
