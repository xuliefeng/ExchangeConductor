...
logger = setup_logger("probit", "../log/app.log")
...
def insert_to_db(data, ref):
    ...
    sql = "INSERT INTO symbols (symbol_name, remark) VALUES (%s, 'probit')"

    cursor.executemany(sql, inst_ids_tuples)

    connection.commit()
    release_connection(connection)
    logger.info(f"{len(filtered_symbols)} record(s) inserted probit")
...
def transform_and_filter_symbols(data, ref):
    ...
    for base_currency in base_symbols:
        ...
        for ref_currency in ref:
            ...
            if matched_symbol in [item['market_id'] for item in data]:
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"probit_unmatched_symbols : {unmatched}")

    with open('probit_unmatched_symbols.txt', 'w') as file:
        for unmatched in unmatched_symbols:
            file.write(f"probit_unmatched_symbols : {unmatched}" + '\n')
