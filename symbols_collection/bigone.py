...
def insert_to_db(data, ref):
    ...
    print(f"{len(filtered_symbols)} record(s) inserted bigone")
...
def transform_and_filter_symbols(data, ref):
    ...
    for base_currency in base_symbols:
        ...
        for ref_currency in ref:
            ...
            if matched_symbol in [item['asset_pair_name'] for item in data]:
    ...
    for unmatched in unmatched_symbols:
        print(f"bigone_unmatched_symbols : {unmatched}")

    with open('bigone_unmatched_symbols.txt', 'w') as file:
        for unmatched in unmatched_symbols:
            file.write(f"bigone_unmatched_symbols : {unmatched}" + '\n')
