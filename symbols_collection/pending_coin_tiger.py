...
logger = setup_logger("coin_tiger", "../log/app.log")
...
def insert_to_db(data, ref):
    ...
    logger.info(f"{len(filtered_symbols)} record(s) inserted coin_tiger")
...
def transform_and_filter_symbols(data, ref):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"coin_tiger_unmatched_symbols : {unmatched}")
