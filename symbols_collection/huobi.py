...
logger = setup_logger("huobi", "../log/app.log")
...
def insert_to_db(data, ref):
    ...
    sql = "INSERT INTO symbols (symbol_name, remark) VALUES (%s, 'huobi')"

    cursor.executemany(sql, inst_ids_tuples)

    connection.commit()
    release_connection(connection)
    logger.info(f"{len(filtered_symbols)} record(s) inserted huobi")
...
def transform_and_filter_symbols(data, ref):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"huobi_unmatched_symbols : {unmatched}")
