...
logger = setup_logger("la_token", "../log/app.log")
...
def insert_to_db(data, ref):
    ...
    sql = "INSERT INTO symbols (symbol_name, remark) VALUES (%s, 'la_token')"

    cursor.executemany(sql, inst_ids_tuples)

    connection.commit()
    release_connection(connection)
    logger.info(f"{len(filtered_symbols)} record(s) inserted la_token")
...
def transform_and_filter_symbols(data, ref):
    ...
    for unmatched in unmatched_symbols:
        logger.info(f"la_token_unmatched_symbols : {unmatched}")

    with open('la_token_unmatched_symbols.txt', 'w') as file:
        for unmatched in unmatched_symbols:
            file.write(f"la_token_unmatched_symbols : {unmatched}" + '\n')
