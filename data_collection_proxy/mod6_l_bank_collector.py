...
def l_bank(symbols, temp_table_name):
    start_time = time.time()
    data = asyncio.run(l_bank_symbols())
    if data:
        found_records = filter_symbols(symbols, data)
        result = asyncio.run(l_bank_depth(found_records))
        insert_to_db(result, temp_table_name)

        end_time = time.time()
        elapsed_time = round(end_time - start_time, 3)
        logger.info(
            f"-------------------------------------------------- l_bank executed in {elapsed_time} seconds. ----- symbols : {len(found_records)} success : {len(result)}")
    else:
        logger.error("Failed to get tickers from l_bank")


async def l_bank_symbols():
            ...
            try:
                async with httpx.AsyncClient(proxies=proxy, verify=False, timeout=20) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"Request failed {symbol} status code {response.status_code} - l_bank")
                        await asyncio.sleep(0.1)

            except Exception as e:
                logger.error(f"Error fetching {symbol}. {repr(e)} - l_bank")
                await asyncio.sleep(0.1)
