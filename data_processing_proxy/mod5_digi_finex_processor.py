...
logger = setup_logger("digi_finex_processor", "log/app.log")


def filter_symbols(data):
    found_records = []
    inst_ids_set = set(item['symbol'] for item in data)
    symbols = get_filtered_symbols_for_exchange('digifinex')
...
def insert_to_db(found_records, temp_table_name):
    ...
    query_temp_table = f"""
        ...
        ) VALUES (%s, %s, %s, %s, %s, %s, '{current_time}', 'digifinex');
    """
    ...
    for symbol_name, result in found_records.items():
        ...
        try:
            reference = str(symbol_name).split('_')[1].upper()
            symbol_name = str(symbol_name).split('_')[0].upper()
