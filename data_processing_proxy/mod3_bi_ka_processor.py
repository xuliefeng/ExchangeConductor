...
logger = setup_logger("bi_ka_processor", "log/app.log")


def filter_symbols(data):
    found_records = []
    inst_ids_set = set(item['trading_pairs'] for item in data)
    symbols = get_filtered_symbols_for_exchange('bika')
    ...
    logger.info(f"bi_ka - symbols       : {len(data)}")
    logger.info(f"bi_ka - symbols found : {len(found_records)}")
